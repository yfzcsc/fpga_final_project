...
class WeightData extends Bundle{
    ...
    val sbx = Vec(10, SInt(18.W))
    val rbx = Vec(10, Bool())
    val bx = Vec(10, SInt(18.W))
    val by = Vec(10, SInt(18.W))
}
...
class Calc6x6(val w: Int, val para_num: Int) extends Module{
    ...
    val _w3 = Wire(Vec(para_num, new TransOutput(w*2+7)))
    val w3 = RegInit(VecInit(Seq.fill(para_num)(0.U.asTypeOf(new TransOutput(w*2+8)))))
    val reg3 = RegInit(VecInit(Seq.fill(para_num)(0.U.asTypeOf(new FirstTransOutput(w*2+9)))))
    ...
    def carry_save(A: SInt, B: SInt, C: SInt): SInt = {
        /*
        val _A = Wire(SInt((C.getWidth).W))
        val _B = Wire(SInt((C.getWidth).W))
        val _C = Wire(SInt((C.getWidth).W))
        _A := A
        _B := B
        _C := C
        val r_and = Wire(SInt((C.getWidth).W))
        val r_xor = Wire(SInt((C.getWidth).W))
    ...
    def final_trans_input(in: FirstTransInput): Unit = {
        ...
        w2m4 := -in.mat_comp(g6(0, 2))+&in.mat_comp(g6(0, 4))
    ...
    def first_trans_output(out: FirstTransOutput, in: TransOutput): Unit = {
        for(i <- 0 to 5)if(i!=4){
            ...
            val w1a2 = RegNext(in.mat_real(g6(1, i))+&in.mat_real(g6(2, i)), 0.S)
            val w1m2 = RegNext(in.mat_real(g6(1, i))-&in.mat_real(g6(2, i)), 0.S)
            ...
            val r_mat_0_i = RegNext(in.mat_real(g6(0, i))<<(2.U), 0.S)
            val r_mat_5_i = RegNext(in.mat_real(g6(5, i))<<(2.U), 0.S)
        ...
        val _w1a2 = RegNext(in.mat_comp(g6(1, 3))+&in.mat_comp(g6(2, 3)), 0.S)
        val _w1m2 = RegNext(in.mat_comp(g6(1, 3))-&in.mat_comp(g6(2, 3)), 0.S)

        val _r34 = RegInit(0.S((in.mat_real(g6(3, 3)).getWidth+1).W))
        val _c34 = RegInit(0.S((in.mat_comp(g6(3, 3)).getWidth+1).W))

        _r34 := in.mat_real(g6(3, 3))-&in.mat_real(g6(3, 4))
        _c34 := in.mat_comp(g6(3, 3))-&in.mat_comp(g6(3, 4))

        val c_mat_0_3 = RegNext(in.mat_comp(g6(0, 3))<<(2.U), 0.S)
        val c_mat_5_3 = RegNext(in.mat_comp(g6(5, 3))<<(2.U), 0.S)
    ...
    def final_trans_output(output: OutputData, in: FirstTransOutput): Unit = {
        for(i <- 0 to 3){
            val w1a2 = RegNext(in.mat_real(g6(i, 1))+&in.mat_real(g6(i, 2)), 0.S)
            val w1m2 = RegNext(in.mat_real(g6(i, 1))-&in.mat_real(g6(i, 2)), 0.S)

            
            val r34 = RegNext(in.mat_real(g6(i, 3))<<1.U, 0.S)
            val c34 = RegNext(in.mat_comp(g6(i, 3))<<1.U, 0.S)

            val r_mat_0_i = RegNext(in.mat_real(g6(i, 0))<<(2.U), 0.S)
            val r_mat_5_i = RegNext(in.mat_real(g6(i, 5))<<(2.U), 0.S)

            output.mat(g4(i, 0)) := carry_save(w1a2, r34, r_mat_0_i)>>2.U
            output.mat(g4(i, 1)) := ((w1m2-&c34)>>2.U)
            output.mat(g4(i, 2)) := ((w1a2-&r34)>>2.U)
            output.mat(g4(i, 3)) := carry_save(w1m2, c34, r_mat_5_i)>>2.U
        }
    }

    val valid_reg = RegInit(VecInit(Seq.fill(10)(false.B)))
