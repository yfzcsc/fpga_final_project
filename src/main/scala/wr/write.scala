...
class BigBankWriteData(val w: Int, val addr_w: Int, val id_w: Int) extends Bundle{
    val data = Vec(48, SInt(w.W))
    val addr = new Address(addr_w, id_w)
}


class SmallBankWriteData(val w: Int, val addr_w: Int, val id_w: Int) extends Bundle{
    val data = Vec(48, SInt(w.W))
    val addr = new Address(addr_w, id_w)
}
...
class Writer(val w: Int, val addr_w: Int, val h_w: Int, val c_w: Int, val id_w: Int) extends Module{
    val io = IO(new Bundle{
        ...
        val to_smallbank = Output(Vec(2, new SmallBankWriteData(w, addr_w)))
        val to_banks = Output(new AddressWriteGroup(addr_w, id_w))
    })

    val gen = RegInit(Reg(new WriteGroup()))
    ...
    when(io.flag_job){
        ...
        when(io.valid_in){
            ...
            for(i <- 0 to 7)
                io.to_smallbank.data(0) := io.in_from_quant.mat(i*8)
            for(i <- 0 to 7)
                io.to_smallbank.data(1) := io.in_from_quant.mat(i*8+7)
